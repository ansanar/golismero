...
def command_plugins(parser, P, cmdParams, auditParams):
    ...
    if testing_plugins:
        ...
        stages = [ (v,k) for (k,v) in iter(STAGES.items()) ]
        stages.sort()
    ...
    try:
        ...
        for info in plugin_infos:
            ...
            if info.plugin_args:
                ...
                for name, default in sorted(iter(info.plugin_args.items())):
...
def command_update(parser, P, cmdParams, auditParams):
    ...
    with golismero.main.testing.PluginTester(autoinit=False) as t:
        ...
        all_plugins = sorted(
            iter(t.orchestrator.pluginManager.load_plugins().items()))
...
def command_run(parser, P, cmdParams, auditParams):
    ...
    try:
        ...
        for plugin_id, plugin_args in iter(cmdParams.plugin_args.items()):
